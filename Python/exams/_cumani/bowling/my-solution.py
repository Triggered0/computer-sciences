...
def main():
    ...
    if not most10:
        ...
        print(f"Most 10: {most10[0]} ({most10[1]["scores"].count("10")} {"times" if most10[1]["scores"].count("10") > 1 else "time"})")

    if not most0:
        print("No player scored 0")
    else:
        print(f"Most 0: {most0[0]} ({most0[1]["scores"].count("0")} {"times" if most0[1]["scores"].count("0") > 1 else "time"})")
