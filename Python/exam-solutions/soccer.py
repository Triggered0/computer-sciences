            ...
            for i, player in enumerate(players):
                ...
                if i > 0:
                    ...
                    for i in range(len(datas)):
                        datas[i] = int(datas[i]) if i > 2 else datas[i] # Convert string to number only once: 1 points
                    ...
                    teams[datas[2]]["ages"].append(2022 - datas[3]) # append ages

            for team in teams:
                teams[team]["average_age"] = sum(teams[team]["ages"]) / len(teams[team]["ages"])
...
def calculate_effs(data):
    for key, _ in data.items():
        data[key]["forward_eff"] = (data[key]["goals"] / data[key]["minutes"]) + (data[key]["assists"] / data[key]["minutes"]) - (data[key]["offsides"] / data[key]["minutes"])

        teams[data[key]["team"]]["players"] = teams[data[key]["team"]].get("players", [])
        teams[data[key]["team"]]["players"].append((data[key]["player"], data[key]["forward_eff"]))

        k = 0 if data[key]["crosses"] == 0 else data[key]["assists"] / data[key]["crosses"] # to prevent division by 0
        
        data[key]["mid_eff"] = (data[key]["interceptions"] + data[key]["ball_recoveries"] + k) / data[key]["minutes"]
...
def main():
    ...
    forward_eff_sort = sorted(players.items(), key=lambda x: x[1]["forward_eff"], reverse=True)
    mid_eff_sort = sorted(players.items(), key=lambda x: x[1]["mid_eff"], reverse=True)
